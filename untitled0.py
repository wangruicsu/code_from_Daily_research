...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 09:20:53 2018
...
#    plt.axis([0, 500, 23.8, 24.2])
    ...
    plt.legend(handles = v_bus, labels = ['bus proposed', 'bus conventional'], loc = 'best',fontsize=15)
...
#    plt.show()
    ...
    plt.figure(figsize = (17,60))
    # 超级电容的SOC：两种方法
    ax2 = plt.subplot(no_pic,2,2)
    plt.grid(True, linestyle = "-.", color = "k", linewidth = "1")
    plt.axis([0, 500, 75, 90])
#    plt.axis([0, 500, 80, 89])
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    X_1,Y_1 = add_guass(t1, df['SC_SOC'].values,sigma = 0.1)
    X_2,Y_2 = add_guass(t1, df['SC_SOC.1'].values,sigma = 0.1)
    SC_SOC = plt.plot(X_1,Y_1, 'g-',linewidth=2)
    plt.legend(handles = SC_SOC, labels = ['SC'], loc = 'best',fontsize=15)
...
#    plt.axis([0, 500, -12, 14])
    ...
    SC_I = plt.plot(X_1,Y_1, 'g-',linewidth=2)
    plt.legend(handles = SC_I, labels = ['SC '], loc = 'best',fontsize=15)
...
#    plt.axis([0, 500, -5, 10])
    ...
    bat_I = plt.plot(X_1,Y_1, 'g--',linewidth=2)
    plt.legend(handles = bat_I, labels = ['bat '], loc = 'best',fontsize=15)
...
#    plt.axis([0, 500, -10, 10])
    ...
    X_1,Y_1 = add_guass(t1, df['bat_I'].values,sigma = 0.1)
    X_2,Y_2 = add_guass(t1, df['SC_I.1'].values,sigma = 0.1)
    I_proposed = plt.plot(X_1,Y_1, 'g--',linewidth=2)
    plt.legend(handles = I_proposed, labels = ['bat'], loc = 'best',fontsize=15)
    ...
    plt.figure(figsize = (17,60))
    # 超级电容的电压：两种方法
    ax6 = plt.subplot(no_pic,2,6)
    plt.grid(True, linestyle = "-.", color = "k", linewidth = "1")
    plt.axis([0, 500, 14.5, 17.5])
#    plt.axis([0, 500, 15.5, 17.5])
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    X_1,Y_1 = add_guass(t1, df['SC_V'].values,sigma = 0.05)
    X_2,Y_2 = add_guass(t1, df['SC_V.1'].values,sigma = 0.05)
    V_SC = plt.plot(X_1,Y_1, 'g--',linewidth=2)
    plt.legend(handles = V_SC, labels = ['SC '], loc = 'best',fontsize=15)
...
#    plt.axis([0, 500, 95.6, 95.9])
    ...
    bat_SOC = plt.plot(X_1,Y_1, 'g--', linewidth=2)
    plt.legend(handles = bat_SOC, labels = ['bat '], loc = 'best',fontsize=15)
    ...
    ax7 = plt.subplot(no_pic,2,7)
    plt.grid(True, linestyle = "-.", color = "k", linewidth = "1")
    plt.axis([0, 500, 15, 16])
#    plt.axis([0, 500, 95.6, 95.9])
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    X_1,Y_1 = add_guass(t1, df['bat_V'].values,sigma = 0.01)
    X_2,Y_2 = add_guass(t1, df['bat_SOC.1'].values,sigma = 0.01)
    bat_SOC = plt.plot(X_1,Y_1, 'g--', linewidth=2)
    plt.legend(handles = bat_SOC, labels = ['bat '], loc = 'best',fontsize=15)
    ax7.set_ylabel('Voltage[v]', fontsize=20)
    ax7.set_xlabel('Time[s]', fontsize=20)
...
#    df =df.head(6000)
    ...
    plt.grid(True, linestyle = "-.", color = "k", linewidth = "2")
    plt.axis([0, 1435.1, 0, 110])
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    I = plt.plot(t1, df['LA92  speed[km/h]'].values, 'g-', linewidth=3)
#    ax1.set_xlabel('Time', fontsize=20)
    plt.legend(handles = I, labels = ['LA92'], loc = 'best',fontsize=15)
    ax1.set_xlabel('Time[s]', fontsize=20)
    ax1.set_ylabel('Speed[Km/h]', fontsize=20)
#    plt.axvspan(0, 600, facecolor='#D8BFD8', alpha=5)
    plt.show()
    ...
    plt.grid(True, linestyle = "-.", color = "k", linewidth = "2")
    plt.axis([0, 1435.1, -70, 50])
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    SC_SOC = plt.plot(t1, df['LA92  power[kw]'].values, 'r-',linewidth=3)
    plt.legend(handles = SC_SOC, labels = ['LA92' ], loc = 'best',fontsize=15)
    ax2.set_xlabel('Time[s]', fontsize=20)
